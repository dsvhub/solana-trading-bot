...
def execute_swap():
    ...
    try:
        ...
        log("Transaction updated with new blockhash.")
        ...
        if txid:
            ...
            log(f"Swap failed: {send_tx_response}")
            send_telegram(f"[ERROR] Swap failed: {send_tx_response.get('error', 'No error details')}")
            position_open = False
