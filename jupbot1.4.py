...
stop_loss_price = 0
take_profit_price = 0
swap_in_progress = False  # New flag to prevent concurrent swaps
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if hasattr(blockhash_resp, 'value') and hasattr(blockhash_resp.value, 'blockhash'):
                blockhash = blockhash_resp.value.blockhash  # Directly use the Hash object
...
def execute_swap():
    ...
    try:
        ...
        if txid:
            ...
            log(f"View on Solscan: https://solscan.io/tx/{txid}{'?cluster=devnet' if 'devnet' in RPC_ENDPOINT else ''}")
            send_telegram(f"✅ Swap Submitted!\nInput: {sol_amount} SOL\nOutput: {int(out_amount_tokens_str)/1e6:.2f} USDC\nTXID: {txid}\nhttps://solscan.io/tx/{txid}{'?cluster=devnet' if 'devnet' in RPC_ENDPOINT else ''}")
            for _ in range(30):
                try:
...
entry_price_input.insert(0, "172.44")
